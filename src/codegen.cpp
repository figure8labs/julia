...
extern "C"
jl_llvm_functions_t jl_compile_linfo(jl_method_instance_t **pli, jl_code_info_t *src, size_t world, jl_cgparams_t *params)
{
    JL_TIMING(CODEGEN);
...
extern "C" JL_DLLEXPORT
void *jl_get_llvmf_defn(jl_method_instance_t *linfo, size_t world, bool getwrapper, bool optimize, jl_cgparams_t params)
{
...
extern "C" JL_DLLEXPORT
void *jl_get_llvmf_decl(jl_method_instance_t *linfo, size_t world, bool getwrapper, jl_cgparams_t params)
{
    // `source` is `NULL` for generated functions.
    // The `isstaged` check can be removed if that is not the case anymore.
    if (linfo->def && linfo->def->source == NULL && !linfo->def->isstaged) {
